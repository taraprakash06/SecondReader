...
export function newMarginDraftId() {
  return Math.random().toString(16).slice(2) + Date.now().toString(16);
}

export function MarginNotesEditor({
  sampleText,
  comments,
  setComments,
  readerName,
  ...
  setComments: React.Dispatch<React.SetStateAction<MarginDraftComment[]>>;
  readerName: string;
  ...
  function addComment() {
    ...
    if (mode === "suggest") {
      const m = `Delete: "${q}"`;
      setComments((prev) => [...prev, { id: newMarginDraftId(), quote: q, message: m }]);
      setQuote("");
      return;
    }
  ...
  if (!sampleText.trim()) {
    return (
      <div className="rounded-2xl border border-dashed border-zinc-300 bg-zinc-50/80 px-4 py-6 text-center text-sm text-zinc-600">
        No excerpt text is available for margin notes.
      </div>
    );
  }
    ...
    <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
      <div className="flex flex-col gap-3 border-b border-zinc-200 bg-zinc-50 px-4 py-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <p className="text-xs font-semibold text-zinc-900">{heading}</p>
